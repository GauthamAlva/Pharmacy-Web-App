...
document.addEventListener('DOMContentLoaded',function(){
    fetch('http://localhost:5000/getAllStaff')
  ...
  function deleteRowByID(id){
      fetch('http://localhost:5000/staffdelete/'+id,{
          method: 'DELETE'
      })
      .then(response =>response.json())
      .then(data =>{
  ...
  updBtn.onclick=function(){
      ...
      fetch('http://localhost:5000/updatestaff',{
          method:'PATCH',
          headers: {
  ...
  srchbtn.onclick=function(){
      ...
      fetch('http://localhost:5000/staffsearch/'+nama)
      .then(response=> response.json())
      .then(data =>loadMedtable(data['data']));
  ...
  fetch('http://localhost:5000/insertstaff',{
      headers: {
          'Content-type':'application/json'
      },
      method:'POST',
  ...
  function  loadMedtable(data){
      ...
      let tableHTML ="";
      data.forEach(function({ID,name,age,gender}){
